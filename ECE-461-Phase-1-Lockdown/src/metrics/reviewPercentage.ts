...
export async function getReviewPercentage(URL: string): Promise<{ score: number, latency: number }> {
    ...
    try {
        ...
        const pullRequests = await fetchJsonFromApi(pullRequestAPI);
        ...
        for (const pr of mergedPRs) {
            ...
            const prDetails = await fetchJsonFromApi(prDetailsAPI);
            ...
            const reviews = await fetchJsonFromApi(reviewsAPI);
