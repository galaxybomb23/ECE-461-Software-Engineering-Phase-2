...
Deno.test(TESTNAME, async () => {
	...
	const pkg     = await queryPackageById(db, id, name, version);
	assertNotEquals(pkg, null, "Package should not be null");
	...
	const success = await updatePackageContent(db, id, newURL, newContent);
	assertEquals(success, true, "Package should be updated");

	// Fetch the package again to verify the content is updated
	const updatedPkg = await queryPackageById(db, id, name, version);
...
Deno.test(TESTNAME, async () => {
	...
	const success = await updatePackageContent(db, id, "https://new-url.com", "New content");
...
Deno.test(TESTNAME, async () => {
	...
	const deletedPkg = await queryPackageById(db, id);
