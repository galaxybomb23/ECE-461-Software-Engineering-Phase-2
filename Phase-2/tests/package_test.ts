...
Deno.test(TESTNAME, async () => {
    ...
    const packageJSON = await handleURL(db, url);

    assertEquals(packageJSON.name, "lodash", "Package name should be lodash");
    assertEquals(packageJSON.version, "4.17.21", "Package version should be 4.17.21");
    assertEquals(packageJSON.url, url, "Package URL should be " + url);

    // Read db and ensure package is uploaded
