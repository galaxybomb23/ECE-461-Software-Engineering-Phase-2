...
Deno.test("PackageTest...", async (t) => {
	await t.step("packageUploadURL-Lodash", async () => {
		...
		try {
			const thing = await handleURL(url, db, false);
			testLogger.info(`Response: ${await thing}\n`);
		} catch (e) {
			testLogger.error(`Error: ${e}`);
		}
	...
	await t.step("packageUploadContent-simplenpmpackage", async () => {
		// pre test setup
		testLogger.info(`TEST: packageUploadContent-simplenpmpackage`);
		const db: DB = await setup();
		...
		const thing = await handleContent(content, undefined, 1, db, false);
		testLogger.info(`Response: ${await thing}\n`);
		// Read db and ensure package is uploaded
		const packages = await db.query(
			"SELECT * FROM packages WHERE NAME = '@clarkio/simple-npm-package'",
		);
		assertEquals(
			packages.length,
			1,
			"Package should be uploaded successfully",
		);
