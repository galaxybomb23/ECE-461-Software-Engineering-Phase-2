...
export async function populateDatabase(
  ...
  try {
      ...
      users: [
        {
          ...
          hashed_password:
            "6af977a963ed05684b582b87299dad067dd2783557a9ebcd6bc209b8229a6eaa", // password is "correcthorsebatterystaple123(!__+@**(A'\"`;DROP TABLE packages;"
          can_search: true,
          can_download: true,
          can_upload: true,
          user_group: "admin",
        ...
        },
        {
          username: "pi",
          hashed_password:
            "f636675642fc2a2b777d34a137210866d3dd1cc5bcdb5ec03406d381adfe3143", // password is "password"
          can_search: true,
          can_download: true,
          can_upload: false,
          user_group: "user",
    ...
    for (const entry of dbentries.packages) {
      await db.query(
        `INSERT OR IGNORE INTO packages (name, url, version, base64_content, readme, license_score, netscore, dependency_pinning_score, rampup_score, review_percentage_score, bus_factor, correctness, responsive_maintainer) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?, ?)`,
        [
          entry.name,
          entry.url,
