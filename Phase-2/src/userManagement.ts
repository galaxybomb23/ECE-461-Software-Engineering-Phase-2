...
export function login(db: DB, username: string, password: string) {
  ...
  let result = db.query(`SELECT hashed_password, password_salt, password_rounds FROM users WHERE username = ?`, [username]);
...
export function delete_account(db: DB, username: string) {
  ...
  return db.changes > 0; // if the username existed then the amount of lines changed should be more than zero. if 
}